...
public class FLTC extends TrajCompressor {
	...
	public FLTC(String tcName, List<Trajectory> trajList,
	...
	public void encode() throws IOException {
		...
		for (Trajectory traj : trajList) {
			...
			for (int i = 2; i < N; i++) {
				...
				point++;
				if (!mostFrequentFollowingLink.containsKey(curPair)) {
					unconnectPoint++;
				}
				if (!mostFrequentFollowingLink.containsKey(curPair)
						|| mostFrequentFollowingLink.get(curPair) != curP
								.getPid()) {
		...
		super.outputData("FLTC", true);
		writeFollowerTable("FLTC_FollowingTable");
		/*super.HuffmanComp("FLTC", "FLTCTree.Comp", "FLTCContent.Comp",
				"FLTCIndex");
		HuffmanAll hf = new HuffmanAll("FLTC_FollowingTable",
				"FLTC_FollowingTable.Comp");
		hf.compress();*/
	}
