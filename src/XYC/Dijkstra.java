...
public class Dijkstra {
	...
	// private HashMap<Pair<Integer, Integer>, List<Integer>> spMap=new
	// HashMap<Pair<Integer,Integer>,List<Integer>>();
	private List<Vertex> vertices;
	private HashMap<Integer, Pair<String, String>> pointIdMap;

	/*
	 * public HashMap<Pair<Integer, Integer>, List<Integer>> getSpMap() { return
	 * spMap; }
	 */
	...
	public Dijkstra(List<Pair<Integer, Integer>> map,
		...
		/*
		 * //spMap for(int i=0;i<vertices.size();i++) { if(i%10==0)
		 * System.out.println(i+"\t"+vertices.size()); Vertex
		 * source=vertices.get(i); computePaths(source);
		 * 
		 * for (Vertex target : vertices) { List<Vertex> path =
		 * getShortestPathTo(target);
		 * 
		 * if(path==null) continue; Pair<Integer,Integer> stPair=new
		 * Pair<Integer,Integer>(source.pid,target.pid); spMap.put(stPair, new
		 * ArrayList<Integer>()); for(Vertex v: path)
		 * spMap.get(stPair).add(v.pid); }
		 * 
		 * resetDis(vertices); } System.out.println("SPMap Done");
		 */
		...
		BufferedReader br = new BufferedReader(new FileReader("dij"));
		String line;
		for (Pair<Integer, Integer> link : map) {
			/*
			 * int npoint =
			 ...
			 * + " " + String.valueOf(link.getSecond()) + " " +
			 * String.valueOf(npoint) + "\n");
			 */
		...
		// bw.close();
		br.close();
		System.out.println("Size:" + fpMap.size());
